...
def main():
    ...
    for epoch in range(last_epoch, config.trainer.max_epoch):
        ...
        checkpoint_path = os.path.join('./checkpoints_path', f'epoch_{epoch}_hvq_my_btad_0.0001.pth')
        torch.save({
            'epoch': epoch,
...
def validate(val_loader, model, model_unet, device):
    ...
    log_file_path = "./checkpoint_paths/log_hvq_my_btad_lr0.0001.txt"

    with open(log_file_path, "a") as log_file:
        for epoch_ in range(4,5): 
            model_path = os.path.join('./checkpoint_paths', f'epoch_{epoch_}_hvq_my_btad_0.0001.pth')
            ...
            if rank == 0:
                config.evaluator.eval_dir_test = f"{config.evaluator.eval_dir}-epoch{epoch_}-{args.lamda}"
            ...
            with torch.no_grad():
                ...
                for idx_, input in enumerate(tqdm(val_loader, desc="Validation Progress")):
                    ...
                    with torch.no_grad():
                        ...
                        input_size = (3, 224, 224)  # 输入通道、高度、宽度
                        ...
                        anomaly_maps = (anomaly_maps1 * args.lamda + pred*(1-args.lamda)) * input["object_mask"].unsqueeze(0).to(device)
