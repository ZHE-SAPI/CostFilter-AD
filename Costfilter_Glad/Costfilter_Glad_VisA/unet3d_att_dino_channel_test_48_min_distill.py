...
class EncoderDiscriminative_att_withDino_3D(nn.Module):
    def __init__(self, in_channels=100, base_width=64, dino_dim=768, min_sim_dim = 4):
        super(EncoderDiscriminative_att_withDino_3D, self).__init__()

        self.down_channel1 = Down_channel_3D(in_channels, base_width * 8)
        self.down_channel2 = Down_channel_3D(base_width * 8, base_width * 2)
        ...
        self.dino_proc1 = DinoFeatureProcessor3D(dino_dim, base_width, target_size=(4, 64, 64))
        self.dino_proc3 = DinoFeatureProcessor3D(dino_dim, base_width * 2, target_size=(4, 16, 16))
        self.dino_proc4 = DinoFeatureProcessor3D(dino_dim, base_width * 4, target_size=(4, 8, 8))
        self.dino_proc6 = DinoFeatureProcessor3D(dino_dim, base_width * 8, target_size=(4, 4, 4))

        # Dino Feature Processors
        self.min_sim_proc1 = Min_Sim_Processor3D(min_sim_dim, base_width, target_size=(4, 64, 64))
        self.min_sim_proc3 = Min_Sim_Processor3D(min_sim_dim, base_width * 2, target_size=(4, 16, 16))
        self.min_sim_proc4 = Min_Sim_Processor3D(min_sim_dim, base_width * 4, target_size=(4, 8, 8))
        self.min_sim_proc6 = Min_Sim_Processor3D(min_sim_dim, base_width * 8, target_size=(4, 4, 4))
    ...
    def forward(self, x, dino_features, min_similarity_map):
        ...
        dino1, dino2, dino3, dino4 = dino_features
        ...
        dino3_proc = self.dino_proc3(dino2)
        ...
        dino6_proc = self.dino_proc6(dino4)
...
class DecoderDiscriminative_att_3D(nn.Module):
    def __init__(self, base_width, out_channels=2, num_classes=2, cls_classes=15):
        ...
        self.conv1 = nn.Conv3d(base_width, base_width, kernel_size=(4, 1, 1))
    ...
    def forward(self, b1, b2, b3, b4, b5, b6):
        ...
        out_48 = self.channel_attention_seg(out)  # 
        # print('out.shape', out.shape) # [4, 48, 1, 64, 64]  out_48
        # out = self.conv2(out).squeeze()  # 
        # print('out.shape', out.shape) 
        out_min, _ = torch.min(out_48, dim=1) # out_48
        ...
        # return out, class_out, [out_48, out_db4]
        return out, class_out, [out_48]
...
class DiscriminativeSubNetwork_3d_att_dino_channel(nn.Module):
    ...
    def forward(self, x, dino_features, min_similarity_map):
        ...
        # 解码器阶段
        output_segment, out_class, features_list = self.decoder_segment(b1, b2, b3, b4, b5, b6)
        # print_memory_usage()
        # 检查 NaN 或 Inf 值
        ...
        if self.out_features:
            return output_segment, out_class, features_list, b2, b3, b4, b5, b6
        else:
            return output_segment, out_class, features_list
...
if __name__ == "__main__":
    ...
    input_tensor = torch.randn(8, 1024, 4, 64, 64).to(device)  # 示例 3D 输入：batch_size=16, channels=12, depth=64, height=256, width=256

    # 创建 DINO 特征
    dino1 = torch.randn(8, 1024, 768 ).to(device)
    dino2 = torch.randn(8, 1024, 768 ).to(device)
    dino3 = torch.randn(8, 1024, 768 ).to(device)
    dino4 = torch.randn(8, 1024, 768 ).to(device)

    dino_features = [dino1, dino2, dino3, dino4]  # DINO特征的四个部分
    min_similarity_map = torch.randn(8, 4, 64, 64).to(device)
    # 计算运行时间
