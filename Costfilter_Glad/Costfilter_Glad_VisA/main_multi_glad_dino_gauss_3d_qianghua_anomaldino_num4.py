...
def parse_args(input_args=None):
    ...
    parser.add_argument("--dino_resolution", type=int, default=256, )
...
def get_class_labels(instance_path, device):
    ...
    for path in instance_path:  # 遍历所有路径
        ...
        # print('path', path) # path ./hdd/Datasets/MVTec-AD/hazelnut/train/good/154.png
...
def test_2dunet_dino_DRAEM(model, dino_model, dino_frozen, val_pipe, weight_dtype, val_dataloader, args, device, class_name, checkpoint_step, log_file, model_num, iftest, len_dataset, a_):
    ...
    with torch.no_grad():
        ...
        for idx_, batch in enumerate(tqdm(val_dataloader, desc="Processing batches")):
            ...
            for i_tem in range(batch_size):
                ...
                with torch.no_grad():
                    ...
                    anomaly_maps1g += F.interpolate(anomaly_map_3.reshape(-1, 1, 32, 32), size=256, mode='bilinear', align_corners=True).to(device)
                    anomaly_maps1g += F.interpolate(anomaly_map_6.reshape(-1, 1, 32, 32), size=256, mode='bilinear', align_corners=True).to(device)
                    anomaly_maps1g += F.interpolate(anomaly_map_9.reshape(-1, 1, 32, 32), size=256, mode='bilinear', align_corners=True).to(device)
                    anomaly_maps1g += F.interpolate(anomaly_map_12.reshape(-1, 1, 32, 32), size=256, mode='bilinear', align_corners=True).to(device)
            ...
            anomaly_map_all_bat = F.interpolate(
                ...
                size=(64, 64),
                mode='bilinear',
                align_corners=False).reshape(2, 1024, 4, 64, 64)
            ...
            output, _ = model(anomaly_map_all_bat.float(), dino_features, min_similarity_map_all_bat.float())
            ...
            # # 2-3
            # anomaly_maps = (gaussian_blur2d(anomaly_maps1*0.9 + anomaly_maps1g12*0.1, kernel_size=(kernel_size, kernel_size), sigma=(sigma, sigma))[:, 0]).unsqueeze(1) * object_mask.to(device) 
            # # 1-1
            # anomaly_maps = (anomaly_maps1*0.95 + anomaly_maps1g12*0.05) * object_mask.to(device)
            # 1-2
            # anomaly_maps = (anomaly_maps1) * object_mask.to(device)
            # # 1-3
            anomaly_maps = (anomaly_maps1*0.9 + anomaly_maps1g12*0.1) * object_mask.to(device)  # anomaly_maps1g12* object_mask.to(device)
            # # 1-4
            # anomaly_maps = (gaussian_blur2d(anomaly_maps1*0.95 + anomaly_maps1g12*0.05, kernel_size=(kernel_size, kernel_size), sigma=(sigma, sigma))[:, 0]).unsqueeze(1) * object_mask.to(device) 
            # print('anomaly_maps.shape', anomaly_maps.shape)
            # # 1-5
            # anomaly_maps = (gaussian_blur2d(anomaly_maps1, kernel_size=(kernel_size, kernel_size), sigma=(sigma, sigma))[:, 0]).unsqueeze(1) * object_mask.to(device) 
            ...
            score = torch.topk(torch.flatten(anomaly_maps, start_dim=1), 10)[0].mean(dim=1)
        ...
        scores = normalize(np.array(scores)).astype(np.float16)
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(
        log_dir='./model/anomalydino/anomalydino_VISA/logs_test_train_2dunet_dino_triloss_cost_anomalydino_num0_rand4')
    if args.train:
        main(args, "")
    else:
        ...
        print(f"./{args.output_dir}/checkpoint-{checkpoint_step}/pytorch_model.bin")
        ...
        pth_files = [
            "./checkpoints_path/epoch_7_540_fscratch_anomalydino_num0_rand4.pth",
        ...
        if test_3d_dino_DRAEM_qianghua:
            start_ = 1
            log_file_path = "./test_costfilter_aonmalydino_1-3-0.90.1+++0326visa.txt"
                    ...
                    for model_path in pth_files:
                        ...
                        for class_name in CLSNAMES:
                            ...
                            results = test_2dunet_dino_DRAEM(model, 
                                ...
                                class_name, checkpoint_step, log_file, 0, test_2d, len(val_dataset), a_)
            ...
            start_ = 1
            log_file_path = "./log_dino_triloss_gai_3d_channel_qianghua_volume_1080_cost_tormin48_anomalydino_num0_rand4.txt"

            with open(log_file_path, "a") as log_file:  # 使用 "a" 模式以追加方式写入
                for epoch_ in range(1,101):
                    for idx_ in range(1080, 1081, 10):

                        model_path = os.path.join('./checkpoints_path', f'epoch_{epoch_}_{idx_}_fscratch_anomalydino_num0_rand4.pth')
