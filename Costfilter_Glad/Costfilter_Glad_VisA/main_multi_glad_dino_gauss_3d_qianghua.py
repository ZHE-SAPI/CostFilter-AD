...
def test_Costfilter_Glad(model, dino_model, dino_frozen, val_pipe, weight_dtype, val_dataloader, args, device, class_name, checkpoint_step, log_file, model_num, iftest, len_dataset, a_):
    ...
    with torch.no_grad():
        ...
        for idx_, batch in enumerate(tqdm(val_dataloader, desc="Processing batches")):
            ...
            anomaly_maps = (anomaly_maps1*0.3 + anomaly_maps1g12*0.7) * object_mask.to(device)
...
def load_vae(vae):
    ...
    if "VisA" in args.instance_data_dir:
        ...
        vae_path = 'model_Glad/vae/pacbank_epoch=245-step=64944.ckpt'
    else:
        vae_path = None
...
if __name__ == "__main__":
    ...
    print(f"代码运行时间train0: {elapsed_time:.4f}秒")  
    start_time = time.time()


    writer = SummaryWriter(log_dir='./model/cost_volume/cost_volume_VISA/test_train_2dunet_dino_triloss_volume_min_randidxhou')
    if args.train:
        main(args, "")
    else:
        ...
        elapsed_time = time.time()- start_time
        print(f"代码运行时间train1: {elapsed_time:.4f}秒")  
        start_time = time.time()
        ...
        elapsed_time = time.time()- start_time
        print(f"代码运行时间train2: {elapsed_time:.4f}秒")  
        start_time = time.time()
        ...
        pth_files = [
            "./checkpoints_path/epoch_1_900_fscratch_channel48min.pth"
        ]
        ...
        if test_3d_dino_DRAEM_qianghua:
            start_ = 1
            log_file_path = "./test++0.30.7_0707.txt"
                ...
                for beta in range(1):
                    for model_path in pth_files:
                        ...
                        for class_name in CLSNAMES:
                            ...
                            fix_seeds(args.seed)
                            data_directory_class = f'./test_save_path_1234_VISA/{class_name}'
                            ...
                            results = test_Costfilter_Glad(model, 
            ...
            start_ = 1
            log_file_path = "./log_test_Costfilter_Glad.txt"

            with open(log_file_path, "a") as log_file:  # 使用 "a" 模式以追加方式写入
                for epoch_ in range(1,12):
                   
                    for idx_ in range(300, 7600):

                        model_path = os.path.join('./checkpoints_path', f'epoch_{epoch_}_{idx_}_fscratch_channel48min_opti.pth')
