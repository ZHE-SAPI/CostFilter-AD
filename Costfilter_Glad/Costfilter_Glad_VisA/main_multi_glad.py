...
from dataset.dataset_multiclass_origin import MVTecDataset
...
def train_save_pt(dino_model, dino_frozen, val_pipe, weight_dtype, val_dataloader, args, device, class_name, checkpoint_step, epoch_num, seed):
    ...
    with torch.no_grad():
        ...
        for idx_, batch in enumerate(tqdm(val_dataloader, desc="Processing batches")):
            ...
            data_dict = {
                ...
                "batch": {
                    "anomaly_images": None,  # 预留位置，稍后可以填充具体数据
                    "anomaly_masks": None,
                    "instance_label": None,
                    "instance_path": None,
                    "idx_hou": None,
            ...
            if idx_ % 10 == 0:
                ...
                sample = reverse_normalization(torch.tensor(anomaly_images))

                sample = sample.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255
                cv2.imwrite(f"/home/ZZ/anomaly/GLAD-main/2d_batch_1_shengchengshuju/train_save_path_{seed}_VISA/train_anomaly_images_epoch_denoise_{seed}_{epoch_num}_{idx_}.png", sample)


                sample = anomaly_masks.squeeze(0).detach().cpu().numpy()[0] * 255
                cv2.imwrite(f"/home/ZZ/anomaly/GLAD-main/2d_batch_1_shengchengshuju/train_save_path_{seed}_VISA/train_anomaly_masks_epoch_denoise_{seed}_{epoch_num}_{idx_}.png", sample)

            file_name = f'/home/ZZ/anomaly/GLAD-main/2d_batch_1_shengchengshuju/train_save_path_{seed}_VISA/train_epoch_denoise_{seed}_{epoch_num}_{idx_}.pt'
    ...
    with torch.no_grad():
        ...
        for idx_, batch in enumerate(tqdm(val_dataloader, desc="Processing batches")):
            ...
            if idx_ % 10 == 0:
                ...
                sample = sample.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255
                cv2.imwrite(f"/home/ZZ/anomaly/GLAD-main/2d_batch_1_shengchengshuju/test_save_path_{seed}_VISA/test_images_epoch_denoise_{class_name}_{epoch_num}_{idx_}.png", sample)


                sample = anomaly_mask.squeeze(0).detach().cpu().numpy()[0] * 255
                cv2.imwrite(f"/home/ZZ/anomaly/GLAD-main/2d_batch_1_shengchengshuju/test_save_path_{seed}_VISA/test_masks_epoch_denoise_{class_name}_{epoch_num}_{idx_}.png", sample)

            file_name = f'/home/ZZ/anomaly/GLAD-main/2d_batch_1_shengchengshuju/test_save_path_{seed}_VISA/test_epoch_denoise_{class_name}_{epoch_num}_{idx_}.pt'
            torch.save(data_dict, file_name)
