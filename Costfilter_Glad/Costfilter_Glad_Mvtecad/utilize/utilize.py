...
def reconstruction(pipe, weight_dtype, args, batch_image, dino_model=None, iftest=None):
    with torch.no_grad():
        ...
        # print('pipe.vae.config.scaling_factor', pipe.vae.config.scaling_factor)
        ...
        # print('pipe.scheduler.config.num_train_timesteps', pipe.scheduler.config.num_train_timesteps) # 650
        # print('dino_model', dino_model)

        if dino_model:
            # print('args.instance_prompt', args.instance_prompt)
            # print('batch_image.shape[0]', batch_image.shape[0])
            # print('args.inference_step', args.inference_step) # 25
            # print('args.denoise_step', args.denoise_step) # 650
            # print('args.input_threshold', args.input_threshold) # 0.45
            # print('args.min_step', args.min_step) # 350

            reconstruct_latents, step, threshold, latents_all, denoise_s_hou = pipe(args.instance_prompt,
                                                       ...
                                                       min_step=args.min_step, if_test = iftest
                                                       )
            return reconstruct_latents, step, latents_all, denoise_s_hou
