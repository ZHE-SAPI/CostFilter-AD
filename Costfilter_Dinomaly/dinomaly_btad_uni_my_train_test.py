...
def train(item_list, model_paths):
    ...
    scheduler = cosine_warmup_scheduler(optimizer, warmup_epochs=5, total_epochs=max_epochs)
    ...
    writer = SummaryWriter(
        log_dir='/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad/logs')
    ...
    for epoch in range(0, max_epochs):
        ...
        for img, anomaly_mask, isnormal, clsname, class_labels in tqdm(train_dataloader):
            ...
            with torch.no_grad():
                ...
                # pred1 = cal_anomaly_maps(en, de, p=p, factor=0.1) # [B, 1, H, W]
                pred1, _ = cal_anomaly_maps_28(en, de, img.shape[-1])
                # print("pred1.shape", pred1.shape) # 12,1,28,28

                if a_ % 20 == 0:
                    for i_ in range(img.shape[0]):
                        sample = nn.UpsamplingBilinear2d(size=(392, 392))(pred1)[i_].detach().cpu().numpy().squeeze() * 255
                        cv2.imwrite(
                            f"/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad/logs/train/pred_dinomaly_{i_}_{a_}.png",
                            sample)
                        ...
                        cv2.imwrite(f"/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad/logs/train/images_dinomaly_{i_}_{a_}.png", sample)


                # pred1 = torch.sqrt(
                #     torch.sum((outputs['feature_rec'] - outputs['feature_align']) ** 2, dim=1, keepdim=True)
                # )
                ...
                # print("torch.mean(min_anomaly_map, dim=1).shape", torch.mean(min_anomaly_map, dim=1).shape) # 16, 1, 64, 64]
                # print("nn.UpsamplingBilinear2d(scale_factor=64/28)(pred1).shape", nn.UpsamplingBilinear2d(scale_factor=64/28)(pred1).shape) # 16,1,64,64

                # print("nn.UpsamplingBilinear2d.shape", (torch.mean(min_anomaly_map, dim=1) + 0.1 * nn.UpsamplingBilinear2d(scale_factor=64/28)(pred1)).squeeze(1).shape) # [16, 64, 64]
                ...
                #     min_similarity_map_all_bat[:, 0, :, :] = ((torch.mean(min_anomaly_map, dim=1) + 0.1 * nn.UpsamplingBilinear2d(scale_factor=64/28)(pred1))/70).squeeze(1)
                # else:
                ...
                #     min_similarity_map_all_bat[:, 0, :, :] = (torch.mean(min_anomaly_map, dim=0) + 0.1 * nn.UpsamplingBilinear2d(scale_factor=64/28)(pred1).squeeze(1))/40
                ...
                # pred = nn.UpsamplingBilinear2d(scale_factor=392/28)(pred1) # [batch_size, 1, 224, 224]
                
                
                # pred_ceshi = (0.1 * nn.UpsamplingBilinear2d(scale_factor=64/28)(pred1).squeeze(1)/40)


                # print("anomaly_map_all_bat.shape", anomaly_map_all_bat.shape)
                ...
                # anomaly_map_all_bat.shape torch.Size([12, 768, 2, 64, 64])
                # min_similarity_map_all_bat.shape torch.Size([12, 2, 64, 64])
                # dino_features[0].shape torch.Size([12, 784, 768])
            ...
            # print('class_labels', class_labels) # [8]
            ...
            torch.nn.utils.clip_grad_norm_(model_unet.parameters(), max_norm=2.0)
            ...
            if a_ % 20 == 0:
                for i_ in range(anomaly_prob.shape[0]):
                    from kornia.filters import gaussian_blur2d
                    sigma = 6
                    kernel_size = 2 * int(4 * sigma + 0.5) + 1
                    sample = anomaly_prob[i_].view(1, 392, 392).unsqueeze(0)
                    sample = np.squeeze(
                        gaussian_blur2d(sample, kernel_size=(kernel_size, kernel_size), sigma=(sigma, sigma))[:,
                        0].detach().cpu().numpy() * 255).astype(np.uint8)
                    cv2.imwrite(
                        f"/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad/logs/train/pred_{i_}_{a_}.png",
                        sample)


                    sample = anomaly_mask[i_].detach().cpu().numpy().squeeze() * 255
                    cv2.imwrite(
                        f"/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad/logs/train/mask_{i_}_{a_}.png",
                        sample)
                    ...
                    sample = sample.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255
                    cv2.imwrite(f"/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad/logs/train/gt_images_test_{i_}_{a_}.png", sample)
        ...
        checkpoint_path = os.path.join('/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad', f'epoch_{epoch}_dinaomaly_my_btadad.pth')
        torch.save({
            'epoch': epoch,
...
def test_only(item_list, model_paths):
    ...
    log_file_path = "/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad/log_dinomaly_my_btad_1.00.0.txt"

    with open(log_file_path, "a") as log_file:
        # custom_epochs = [23,16,19,15,17] # _002
        # custom_epochs = [0,3,1,4,6,13,15] # _001
        # custom_epochs = [16]
        # for epoch_ in custom_epochs:
        state_dict = torch.load(model_paths[0], map_location=device)
        model.load_state_dict(state_dict)

        for epoch_ in range(0,max_epochs): 
        

            # model_path = os.path.join('/home/ZZ/anomaly/UniAD-main_visa_my/experiments/MVTec-AD/checkpoint_paths', f'epoch_{epoch_}_hvq_my_visa.pth')
            # model_path = os.path.join('/home/ZZ/anomaly/UniAD-main_visa_my/experiments/MVTec-AD/checkpoint_paths/epoch_16_hvq_my_visa.pth')
            # 打开 log 文件，并将每次模型测试结果写入其中
            model_path_unet = os.path.join('/home/customer/Desktop/ZZ/anomaly/Dinomaly-master/checkpoint_paths/btadad', f'epoch_{epoch_}_dinaomaly_my_btadad.pth')
            import time
            ...
            for item, test_data in zip(item_list, test_data_list):
                test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=batch_size, shuffle=False,
                                                              num_workers=4)
                results = evaluation_batch(model, model_unet, test_dataloader, device, max_ratio=0.01, resize_mask=256)
...
# nohup python dinomaly_btad_uni_my_train_test.py --train > dinomaly_btad_uni_my_train.log 2>&1 &


# nohup python dinomaly_btad_uni_my_train_test.py > dinomaly_btad_uni_my_test1.00.0.log 2>&1 &
