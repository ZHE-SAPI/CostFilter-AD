...
def calculate_flops(model, input_tensor, dino_features, min_similarity_map):
    """
    计算模型的FLOPs和参数量
    """
    # 使用thop.profile统计模型的FLOPs和参数量
    flops, params = profile(
        model,
        inputs=(input_tensor, dino_features, min_similarity_map),
...
class EncoderDiscriminative_att_withDino_3D(nn.Module):
    def __init__(self, in_channels=100, base_width=64, dino_dim=768, min_sim_dim = 4):
        ...
        self.dino_proc1 = DinoFeatureProcessor3D(24, base_width, target_size=(4, 64, 64))
        self.dino_proc3 = DinoFeatureProcessor3D(32, base_width * 2, target_size=(4, 16, 16))
        self.dino_proc4 = DinoFeatureProcessor3D(56, base_width * 4, target_size=(4, 8, 8))
        self.dino_proc6 = DinoFeatureProcessor3D(160, base_width * 8, target_size=(4, 4, 4))
...
if __name__ == "__main__":
    ...
    # 创建 3D 网络模型
    model = DiscriminativeSubNetwork_3d_att_dino_channel(in_channels=196, out_channels=2, base_channels=48).to(device)
    model.eval()  # 设置为评估模式
    ...
    input_tensor = torch.randn(8, 196, 4, 64, 64).to(device)  # 示例 3D 输入：batch_size=16, channels=12, depth=64, height=256, width=256

    # 创建 DINO 特征
    dino1 = torch.randn(8, 196, 24 ).to(device)
    dino2 = torch.randn(8, 196, 32 ).to(device)
    dino3 = torch.randn(8, 196, 56 ).to(device)
    dino4 = torch.randn(8, 196, 160 ).to(device)
