...
class HVQ_TR_switch_OT(nn.Module):
    ...
    def forward(self, inputs, device):
        ...
        print('org_feature.size()', org_feature.size())
        ...
        for i in range(batch_size):
            ...
            # print('tmp_label0', tmp_label)
            feature_rec_tokens[:, i, :] = self.output_proj_list[tmp_label](dec[:, i, :])


        print("feature_rec_tokens.shape", feature_rec_tokens.shape) # 196, b, 272
        ...
        print("min_anomaly_map.shape", min_anomaly_map.shape) # [batch_size, 4, 32, 32]
        anomaly_map_all_bat = torch.stack(anomaly_map_all_bat, dim=0).squeeze(2) # [4, batch_size, 14, 14, 196]
        # anomaly_map_all_bat = F.interpolate(anomaly_map_all_bat.permute(0, 1, 4, 2, 3).view(-1, pre_min_dim, H, W), size=64, mode='bilinear', align_corners=True)
        ...
        # print("min_anomaly_map.shape", min_anomaly_map.shape) # [batch_size, 1, 224, 224]
        ...
        print("pred.shape", pred.shape) # [batch_size, 1, 14, 14]
        # add OT
        # if not self.training:
        print("+++++")
        

        print("ot_score.shape", ot_score.shape) # [1, 196]

        pred = pred + 80.0 * torch.reshape(ot_score, shape=(-1, 14, 14)).unsqueeze(1)
        # print("Max value1111:", pred.max().item())
        # print("Min value1111:", pred.min().item())
        # print("+++++++++++")
        pred = self.upsample(pred)

        print("Max value:", (0.1*pred).max().item()) 
        print("Min value:", (0.1*pred).min().item())

        # pred = min_anomaly_map + 0.1*pred

        
        # pred = self.upsample(pred)
        # # print("ot_score.shape", torch.reshape(ot_score, shape=(-1, 14, 14)).unsqueeze(1).shape) # [1, 1, 14, 14]
        # print("pred.shape", pred.shape) # [1, 1, 224, 224]
