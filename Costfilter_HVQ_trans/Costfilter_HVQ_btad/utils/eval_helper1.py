...
class EvalMetric:
    def __init__(self, preds, labels, masks=None):
        ...
        print('np.sum(self.masks)', np.sum(self.masks))
        print('np.sum(self.preds)', np.sum(self.preds))
        print('np.sum(self.scores)', np.sum(self.scores))
        print('np.sum(self.labels)', np.sum(self.labels))
    ...
    def eval_pixel_metrics(self):
        ...
        print('self.masks.ravel()', self.masks.ravel())
        print('self.masks.ravel().dtype', self.masks.ravel().dtype)
        print('self.masks.ravel() max:', self.masks.ravel().max())
        print('self.masks.ravel() min:', self.masks.ravel().min())


        # mask_ravel = self.masks.ravel()
        # invalid_vals = mask_ravel[(mask_ravel != 0.0) & (mask_ravel != 1.0)]

        # if len(invalid_vals) > 0:
        #     print("Found values not equal to 0.0 or 1.0 in self.masks.ravel():")
        #     print(np.unique(invalid_vals))
        # else:
        #     print("All values in self.masks.ravel() are 0.0 or 1.0.")
        self.masks = (self.masks >= 0.5).astype(float)


        precisions_pixel, recalls_pixel, _ = precision_recall_curve(self.masks.ravel(), self.preds.ravel())
        print(f"precisions_pixel: {precisions_pixel}")
        print(f"recalls_pixel: {recalls_pixel}")
        print(f"precisions_pixel + recalls_pixel: {precisions_pixel + recalls_pixel}")
    ...
    for clsname in clsnames:
        ...
        for fileinfo, pred, mask in zip(fileinfos, preds, masks):
            if fileinfo["clsname"] == clsname:
                ...
                # print('((mask.reshape(mask.shape[0], -1).sum(axis=0)) != 0).astype(int)', ((mask.reshape(mask.shape[0], -1).sum(axis=0)) != 0).astype(int))
        ...
        print('labels_cls.shape', labels_cls.shape)
        print('preds_cls.shape', preds_cls.shape)
        print('masks_cls.shape', masks_cls.shape)

        print('np.sum(labels_cls)', np.sum(labels_cls))
        print('np.sum(preds_cls)', np.sum(preds_cls))
        print('np.sum(masks_cls)', np.sum(masks_cls))
        ...
        # # 保存到本地
        # with open(f"/home/customer/Desktop/ZZ/anomaly/MY_HVQeriment_VISA/H+O_VISA_curve/filtered_data-H-{clsname}.pkl", "wb") as f:
