...
def main():
    ...
    for epoch in range(last_epoch, config.trainer.max_epoch):
        ...
        checkpoint_path = os.path.join('./checkpoint_paths', f'epoch_{epoch}_hvq_my_mvtecad.pth')
        torch.save({
            'epoch': epoch,
...
def validate(val_loader, model, model_unet, device):
    ...
    log_file_path = "./checkpoint_paths/log_costfilter_hvq_mvtecad.txt"

    with open(log_file_path, "a") as log_file:
        for epoch_ in range(1):
            model_path = os.path.join('./checkpoint_paths/epoch_23_hvq_my_mvtecad.pth')
            ...
            if rank == 0:
                config.evaluator.eval_dir_test = f"{config.evaluator.eval_dir}"
            ...
            with torch.no_grad():
                ...
                for idx_, input in enumerate(tqdm(val_loader, desc="Validation Progress")):
                    ...
                    with torch.no_grad():
                        ...
                        anomaly_maps = (anomaly_maps1 + pred*0.02) * input["object_mask"].unsqueeze(0).to(device)
