...
parser.add_argument("--config", default="./config_visa_woOT.yaml")  
...
def main():
    ...
    for epoch in range(last_epoch, config.trainer.max_epoch):
        ...
        checkpoint_path = os.path.join('./checkpoint_paths', f'epoch_{epoch}_hvq_my_visa.pth')
        torch.save({
            'epoch': epoch,
...
def train_one_epoch(
    ...
    for idx_, input in enumerate(tqdm(train_loader)):
        ...
        # forward
        with torch.no_grad():
            anomaly_map_all_bat, min_similarity_map_all_bat, dino_features, _, _ = model(input, device, out_pred_ =False)
        ...
        gamma = compute_dynamic_gamma(cls_out, input["clsname"], class_labels.float(), 224, num_classes=15).to(
            device)
...
def validate(val_loader, model, model_unet, device):
    ...
    log_file_path = "./checkpoint_paths/log_costfilter_hvq_visa.txt"

    with open(log_file_path, "a") as log_file:
        for epoch_ in range(1):
            model_path = os.path.join('./checkpoint_paths/costfilter_hvqtrans_visa.pth')
            ...
            if rank == 0:
                config.evaluator.eval_dir_test = f"{config.evaluator.eval_dir}-epoch{epoch_}-_*0.01"
            ...
            with torch.no_grad():
                ...
                for idx_, input in enumerate(tqdm(val_loader, desc="Validation Progress")):
                    ...
                    with torch.no_grad():
                        anomaly_map_all_bat, min_similarity_map_all_bat, dino_features, pred, outputs = model(input, device, out_pred_ =True)
                        ...
                        anomaly_maps = (anomaly_maps1 + pred*0.01) * input["object_mask"].unsqueeze(0).to(device)
                        outputs["pred"] = anomaly_maps


                    dump(config.evaluator.eval_dir_test, outputs)
                    pred_max = outputs["pred"].max().item()
                    pred_min = outputs["pred"].min().item()

                    # print('input["object_mask"].shape', input["object_mask"].shape)
